    ...
    <div class="wrappers">
        <div class="saleMain">
            <div class="saleMainRight">
                ...
                <div class="saleMainRightTitle">
                    <img src="<?php echo PATH ?>/image/icon/group39.png" alt="">
                    <?php
                    $title1 = tirestan_get_option('search_settings')[0]['meta_key_title1'];
                    if (!empty($title1)) {
                        echo '<h3>' . $title1 . '</h3>';
                    } else {
                        echo '<h3>پرفروش ترین ها</h3>';
                    }
                    ?>
                </div>
                <a href="<?php echo tirestan_get_option('search_settings')[0]['meta_key_link2'] ?>">
                    <button  class="saleMainButton">مشاهده</button>
                </a>
            </div>
            <div class="saleMainLeft">
                <?php
                $meta_key = tirestan_get_option('search_settings')[0]['meta_key2'];
                $cat_key = tirestan_get_option('search_settings')[0]['meta_key_cat_name2'];
                if (!empty($cat_key)) {
                    $replywp_args = array(
                        'post_type' => 'product',
                        'posts_per_page' => 6,
