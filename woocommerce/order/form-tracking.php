...
<form action="<?php echo esc_url( get_permalink( $post->ID ) ); ?>" method="post" class="woocommerce-form woocommerce-form-track-order track_order">
	...
	<p class="form-row form-row-last"><label for="billing_meli"><?php echo esc_html( 'کد ملی' ); ?></label> <input class="input-text" type="text" name="billing_meli" id="billing_meli" value="<?php echo isset( $_REQUEST['billing_meli'] ) ? esc_attr( wp_unslash( $_REQUEST['billing_meli'] ) ) : ''; ?>" placeholder="<?php echo esc_attr( 'کد ملی که در هنگام سفارش وارد کرده اید.' ); ?>" /></p><?php // @codingStandardsIgnoreLine ?>
	<div class="clear"></div>
